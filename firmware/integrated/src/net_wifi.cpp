...
// Next time wifi_tick() should run its reconnect check.
...
void wifi_begin() {
    ...
    while (WiFi.status() != WL_CONNECTED &&
        ...
        delay(250);   // boot-time only; the main loop() is never blocked.
        Serial.print(".");
    }
    Serial.println();
...
void wifi_tick() {
    ...
    // Link is down. Kick a non-blocking reconnect; association completes in
    // the background and we re-check on the next cadence.
