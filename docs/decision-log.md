...
| [DL-196](#dl-196) | 2026-08-24 | **Vision phase, Piece 1 — camera node accepts a Pi-triggered capture over MQTT.** First slice of the top-down-camera rework: the plan is 5 grow-light-OFF measurement windows/day (glare-free, feed the calibrated metric) + a 30-min lit time-lapse (visual record only), all Pi-orchestrated. Only the Pi knows the wall clock and owns the grow-light, so capture becomes Pi-triggered (single authority, no cross-clock drift). Added MQTT to the camera node (it was WiFi+HTTP only): connects to the broker as client `camera`, retained presence on `plant/status/camera` + Last-Will, subscribes to `plant/cmd/capture`. Payload is the capture CONTEXT ("dark"/"lit"); the callback only latches a one-shot + context (no blocking capture inside the MQTT callback — mirrors the WROVER remote-start/abort pattern), and the loop does the capture+POST, echoing the context as the `X-Capture-Context` HTTP header for the receiver to tag (Piece 2). Self-timer kept as a 2h resilience FALLBACK, tagged "fallback" so uncoordinated frames never pollute the calibrated metric (D2). Mirrors WROVER net_mqtt exactly; PubSubClient ^2.8. Braces/parens balanced, signatures consistent, include graph resolves. **Firmware — awaiting reflash of the XIAO; verify: `mosquitto_pub plant/cmd/capture -m dark` triggers a capture+POST** | Active |
| [DL-197](#dl-197) | 2026-08-24 | **Vision phase, Piece 2a — receiver tags each capture with its context.** The node now sends `X-Capture-Context` (DL-196); the Pi receiver reads it and stores it so downstream can keep only the grow-light-OFF `dark` frames for the calibrated metric. Added a `context TEXT` column to `camera_readings` via the existing idempotent `ALTER TABLE ADD COLUMN` migration (DL-079 pattern, auto-upgrades the live DB); `record()` and the POST handler carry it; absent header → `"unknown"` (never mistaken for calibration-grade `dark`); response + serial log echo it. All in-window captures are stored regardless of context (filtering to `dark` happens later at the metric step). Backward-compatible: every `camera_readings` query selects explicit columns (no `SELECT *`), so the DL-194 dashboard query and the DL-195 tests are unaffected. Updated `hub/09-camera/README.md` for the tagged POST. py_compile OK; 73 tests green. Deploy: scp receiver + restart plant-image-receiver (column auto-migrates) | Active |
| [DL-198](#dl-198) | 2026-08-24 | **Vision phase, Piece 2b — camera-node firmware doc-banner cleanup (comment-only, no reflash).** The DL-196 code change left the firmware's file banners describing the *old* self-timed model: `main.cpp`, `platformio.ini`, and `README.md` still said "on a timer," "cadence hourly," "node carries no clock," and "MQTT capture event / presence is a later slice" — all false since capture is now Pi-triggered over MQTT with a slow fallback. Rewrote the four banners to the current model (Pi-published `plant/cmd/capture` trigger, context tagging, self-timer as resilience fallback, presence over MQTT), preserving accurate context (DL refs, WiFi-non-fatal, Pi-side photoperiod gating) and the legitimately-still-future watchdog slice. Verified comment-only: the `.cpp`/`.ini` diffs contain zero non-comment lines → byte-identical binary, **no reflash**. 73 tests unaffected (no hub/python touched) | Active |
...
| [DL-203](#dl-203) | 2026-09-02 | **Raise the watering band 20-40% -> 60-80% (plant was chronically under-watered).** Two weeks of soil data showed the plant watered once (Aug 20 -> ~71%) then a linear ~2.5%/day dry-down for 13 straight days to 28% today, never re-triggering because it never hit the old 20% floor -- and physical signs (browning leaf edges, curled-but-green leaves, dried leaves) confirmed water stress. The plant drew down the Aug-20 ~71% healthily, so it tolerates/prefers the higher range. Changed `TRIGGER_PCT` 20->60 and `TARGET_PCT` 40->80 (config.h only; dose constants unchanged). At 28% the plant is well below the new 60% trigger, so it waters on the first cycle after flashing. **Watch the first cycle or two:** the only prior equilibration data (100mL -> ~70%) was at the old 40% target; the supplement loop will now chase 80%, and delayed wicking could overshoot -- confirm where it actually settles before tuning dose sizes. Firmware verified: only 2 thresholds + comments changed, braces balanced, 31 C++ checks pass. Awaiting reflash | Active |
| [DL-204](#dl-204) | 2026-09-03 | **Fix premature watering fault on drought recovery — `ABSORB_RISE_PCT` 7->2.** After DL-203 raised the band to 60/80, the first watering of the bone-dry plant (28% after 13 days) dosed 100mL, the probe rose only ~3% (28->30.7 EMA), and the old 7% "absorbed" bar misread that real absorption as a stall -> GRACE -> STOP_FAILED, so it faulted after a single dose instead of supplementing toward target. Root cause: the 7% threshold was over-fit to the normal moisture range; in very dry (or near-full) media the same dose moves the probe less. Confirmed by a later session in the normal range that rose >=7% and correctly supplemented to 80% in 2 doses (150mL) with no fault. Fix: lower `ABSORB_RISE_PCT` to 2% -- any real positive rise counts as absorption; only ~0% (dead pump/empty reservoir) still faults. Chosen for asymmetric cost: a false stall strands a thirsty plant, a false-absorbed just adds one capped 50mL supplement, so bias toward watering. C++ tests: added 6 cases at the production 2.0 threshold using the real numbers (3% dry-recovery -> SUPPLEMENT, 8% normal -> SUPPLEMENT, ~0.5% failure -> GRACE/STOP_FAILED, near-target -> DONE); mutation-verified (reverting to 7.0 fails the 3% case). 37 C++ checks + 95 Python green. Awaiting reflash | Active |
| [DL-205](#dl-205) | 2026-09-03 | **Fix "dark" captures coming back lit — stale camera frame buffer.** Every grow-light-OFF capture showed the light ON despite correct scheduling, a corrected clock, and the enforcer verifiably holding the light off (15:58 OFF -> 16:04 ON, capture at 16:00). An on-demand test settled it: light physically OFF (watched by eye, Shelly `was_on:true`->off), capture fired, image still lit -> the node was returning a **stale buffered frame** from before the light went off. Cause: `camera.cpp` used `grab_mode = CAMERA_GRAB_WHEN_EMPTY` with `fb_count=2`, so `esp_camera_fb_get()` handed back a queued pre-light-off frame; `camera_capture()` did a single grab with no flush. Fix: `grab_mode = CAMERA_GRAB_LATEST` + `camera_capture()` now drains the queued buffers (grab+return fb_count times, 120ms between so AE/AGC re-adapts to the dark scene) before grabbing the fresh frame it returns. Framebuffer discipline preserved (no leak). Explains the whole "dark frames look lit" saga; timezone (DL earlier today) was a separate real bug. Camera-node reflash; braces/parens balanced, only camera.cpp changed | Active |
| [DL-206](#dl-206) | 2026-09-03 | **Purge corrupted/test camera rows from plant.db (data hygiene for calibration).** After DL-205 fixed the stale-buffer bug, removed the data it had polluted so the eventual dark-frame calibration/ML set is clean: deleted 44 rows + their JPEGs -- all 33 `dark` rows (Aug 27-Sep 3, every one a pre-fix stale-buffer LIT frame mislabelled dark), the 2 manual `darktest`/`darktest2` test rows, and 9 `fallback` rows (node self-timer, uncontrolled lighting, never metric-valid). **Kept** the 126 `lit` time-lapse rows and the 948 empty-context rows (the Jun 22-Aug 25 pre-tagging growth baseline -- valuable history, and their empty context cleanly excludes them from any dark-frame filter). Live-DB op (no repo change): SELECT-confirmed 44 before deleting, removed image files first then rows, `changes()`=44, after-state = only `lit`(126) + empty(948). Net effect: `context='dark'` now contains only genuinely-dark frames captured from the DL-205 fix onward | Active |
| [DL-207](#dl-207) | 2026-09-03 | **README currency pass for the vision phase + watering changes.** Brought the top-level README in line with the system as built (docs only). Updated: the "Next" roadmap row (the glare-free dark-window capture pipeline is built + hardware-validated; what remains is greenness-metric calibration on those frames + adaptive lighting); the autonomous-watering bullet (60->80% band with the DL-203 rationale, DL range 104->204); the grow-light bullet (notes the five enforcer-held dark measurement windows, DL-199); the repo layout (hub range 01…11 -> 01…13-capture-scheduler + capture orchestrator in the description); and the push-alerting bullet (adds the 9pm end-of-day summary with net soil-moisture change, DL-202). Deliberately framed the capture *architecture* as done and the *calibration/analysis* as next -- accurate, since the clean pipeline is one day old and multi-day collection hasn't run yet. Left Phase 4 row as-is (accurate for the original camera bring-up). No terminology regressions | Active |
...
<a id="dl-125"></a>
### DL-125 — Probe is blind to bottom-watering; calibration goes empirical; 100 mL study dose

**Date:** 2026-08-16 · **Status:** Active — study/finding; firmware is a temporary study-dose value.

**Finding.** The DL-124 **150 mL** dose was clearly absorbed and used — the tray drew down within ~3 h (observed) and the plant put on new canopy growth — yet over the following ~2 days the soil probe never rose above ~6% (raw held ~2533–2557, essentially flat). Three independent signals (tray drained, new growth, plant recovered) say the water reached the roots; the probe says it didn't. When physical reality and the sensor disagree this starkly, the sensor is what's wrong: **this probe does not register a tray-fed (bottom) dose that wicks to the root zone.** Most likely the water channels up through the root zone (lower) while the probe's sensing length sits in a drier mid/upper pocket — consistent with the probe being intentionally placed high enough that its electronics stay out of the soil (correct for the hardware, but it under-reports bottom-watering). The probe is *not* moved: it is correctly placed for its electronics, and no single point represents an unevenly-wetted bottom-watered column. It still reads sharply for *top*-watering (not how this system waters).

**Why the DL-124 run emergency-stopped (mechanism).** `evaluate()` decides via `rise = moist_ema - dose_before`, a probe measure. With the probe blind, `rise` stays ~0, so the loop can never take the "absorbed" or "target reached" branch and always falls through to stall → grace → **fail → emergency stop** (exactly the ntfy trail seen). The single-dose *delivery* worked; the probe-gated *evaluation* cannot.

**Consequences for calibration (decided).**
1. **Keep the wide 2585 / 1700 mapping** — do not compress it to the 0–10% range the probe shows after bottom-watering (that would encode the sensor's blind spot as truth).
2. **Define the healthy band empirically** — treat "what the probe reads when the plant is verifiably thriving (tray drawn down + new growth + camera health)" as the operational reference, rather than assuming textbook percentages apply at this probe position.
3. **The probe alone cannot drive autonomy** — it reads ~4–6% both when bone-dry-and-wilting *and* when freshly-watered-and-thriving, so it cannot distinguish "needs water" from "just watered." A safe autonomous trigger needs a **second signal**: watering-event memory (don't re-water what was just watered) and/or the **camera** as a health cross-check. Tray-drawdown (absorption) is a more reliable "did watering work" signal than the probe.

**Direction (longer arc).** Build the system's own hydraulics model from labeled per-dose data — Δmoisture with respect to *time* and *volume dispensed* — so watering becomes predictive (dose→response known) rather than threshold-reactive; eventually an ML layer could predict need and manage watering/light. Each isolated, measured dose is a data point toward that.
...
**Why it matters.** After DL-159/160 the logic was *extracted and tested*, but `fsm.cpp` still ran its own parallel inline copies — so the tests and the firmware could silently drift. This step makes the firmware **call** the tested functions, so the code under test is the code that runs.

**Behavior-identical.** Each replacement was checked for exact equivalence:
- `begin_dose` — `clamp_dose` returns 0 for both "budget exhausted" and "non-positive request"; the wrapper reproduces the original's split (capped→STOPPED only when `session_ml >= SESSION_CAP_ML`, else silent return).
- plateau — `is_plateau` folds the window+slope test; the "window elapsed but not flat → re-arm reference" branch is preserved as an explicit `else if`.
- freshness — `(unsigned long)(now - last_valid_soil_ms)` keeps the 32-bit rollover-safe arithmetic (DL-131) on-target.
- `evaluate` — the `Decision` enum maps 1:1 to the original if/else arms.
...
<a id="dl-177"></a>
### DL-177 — Audit fixes F12 + F13: query correctness across the hub

**Date:** 2026-08-20 · **Status:** Active — hub-only; the SQL-correctness pair.

**F12 — same-second tie ordering.** `ts` is stored as 1-second-resolution ISO text, so multiple rows routinely share a `ts`. `ORDER BY ts DESC LIMIT 1` then returns an arbitrary member of the tie, not necessarily the newest — so "latest reading" helpers could silently return a stale row when two arrive in the same second (common during a dose). Fixed by ordering on `id` (SQLite's unique monotonic rowid) instead: converted plantctl's `_latest` helper and its 4 inline `ORDER BY ts DESC LIMIT 1` queries, plus the dash_common `camera_readings` latest query. All "latest single row" reads in the hub now use `ORDER BY id DESC`.

**F13 — missing device predicates.** Several EAV queries filtered only on `sensor=`/`metric=` without a `device=`. Because each sensor publishes on its own topic (`plant/sensors/<device>` → lux=`bh1750`, moisture/soil_raw=`soil`, air=`bme280`; state/pump=`wrover`), a sensor name is currently unique to one device, so nothing is broken today — but an added device with an overlapping name would silently pollute the result. Pinned each to its verified device: plantctl pump-on query (`device='wrover'`), alerter `_latest_lux` and `_lux_ever_seen` (`device='bh1750'`), alerter external-water pump-count (`device='wrover'`), alerter `_soil_avg` (`device='soil'`). The DL-173 `_lux_ever_seen` helper is pinned here too, and its 3 F7 tests updated to insert lux under `device='bh1750'`.
...
<a id="dl-184"></a>
### DL-184 — Pin CI test dependencies for reproducible builds

**Date:** 2026-08-20 · **Status:** Active — P2 hygiene (the one P2 item worth doing regardless of threat model).

**Why.** `requirements-dev.txt` declared `pytest>=8.0` / `paho-mqtt>=1.6,<3.0`. Ranges mean a future CI run (or a fresh local `pip install`) could resolve a newer version whose behaviour differs from what the tests were written and validated against — a silent-drift source that turns "CI was green last month" into a non-reproducible claim.

**Change.** Pinned to the exact versions the suite is known-good on: `pytest==9.1.1`, `paho-mqtt==2.1.0`. Version bumps are now a deliberate edit (re-run the suite, then commit the new pin) rather than passive drift. The GitHub Actions themselves were already pinned appropriately (`actions/checkout@v4`, `actions/setup-python@v5`, `python-version: "3.12"`).

**Scope note.** This is test/CI reproducibility only; it does not touch the Pi runtime deps (managed on-device).
...
<a id="dl-188"></a>
### DL-188 — Drop the retention-bound `_lux_ever_seen` guard (F7 hardening)

**Date:** 2026-08-20 · **Status:** Active — hub-only; closes the audit's `_lux_ever_seen` finding.

**The hole.** The DL-173 F7 fix added `_lux_ever_seen` to tell a genuinely lux-less firmware (the old calibration harness — no I²C, stay quiet) from a lux-capable board whose BH1750 has since died (a real, alertable fault). But the helper answered "ever seen lux?" by scanning `sensor_readings`, which retention prunes at 30 days. So if the BH1750 died and stayed dead beyond the window, its last lux row aged out, `_lux_ever_seen` returned False, and F7's masking quietly came back — a dead sensor silently stopped alerting. (All three DB tables are pruned — `mqtt_messages` 7 d, `system_status` 14 d, `sensor_readings` 30 d — so there was no existing longer-lived table to lean on either.)

**Fix — Option 2 (remove the guard), chosen over Option 1 (add a durable `device_capabilities` marker table).** The guard only ever existed to distinguish the harness from integrated, and the harness is retired (DL-157, do-not-flash) — the only board flashed to the plant is the lux-capable integrated firmware. And a BH1750 that *never* worked is itself a fault worth surfacing, not something to stay quiet about. So the guard now protects against a device that doesn't exist while introducing a retention bug. Removed it: F7 alerts whenever the board is demonstrably alive (fresh soil) and no lux has arrived for 30 min during lit hours — no history dependency, no retention hole. Deleted the `_lux_ever_seen` helper and its three tests; updated the branch comment to explain the removal.

**Future note.** If a genuinely lux-less node is ever added (e.g. a new bench harness), it would be re-gated at that point — harnesses remain disposable bench tools that can be built anytime; this only removes an obsolete carve-out for one that's retired.
...
<a id="dl-191"></a>
### DL-191 — Commit the supervised float-divider bench sketch + record the float-sensing decision

**Date:** 2026-08-20 · **Status:** Active — repo hygiene + a captured design decision. No build/test/behavior change.

**What it is.** `firmware/test-sketches/16-float-divider/` was a complete but untracked PlatformIO diagnostic in the working tree. It reads a resistor-divider float sensor on GPIO35 and resolves **three** states — empty (~0), water (~2048), and wire-cut/disconnected (~4095) — to test giving the reservoir float the same supervised disconnect-detection the leak sensor received under P0-5 (DL-140, the 100k pull-up on GPIO39).

**The decision it records.** The production integrated firmware kept the **two-state digital float** (`FLOAT_PIN=27`, `INPUT_PULLUP`, closed = empty), not this three-state divider. Supervised-disconnect sensing was retained only where it earns its complexity — the leak sensor, where a floating disconnected input is the dangerous failure. A disconnected float is instead caught by the closed-loop watering watchdog (pump runs, moisture doesn't rise → `watering_fault`), so a third electrical state wasn't needed. This sketch is the bench evidence for that "measured and set aside" call — exactly the kind of considered-alternative the log exists to preserve.

**Why commit it.** By the repo's own convention every `test-sketches/` entry (01–16) is tracked with a README; leaving a finished, informative sketch untracked was the only inconsistency. Nothing depended on it (no correctness impact), but committing it turns a loose end into documented engineering judgment — more valuable for a portfolio than a repo that shows only the paths taken.

**Fixes applied while committing.** The `platformio.ini` header comment said GPIO32 while `main.cpp` uses GPIO35 (32 is `BTN_ABORT`); corrected the comment to match the code. Added a `README.md` in the 01–15 house style that frames the sketch honestly as explored-not-adopted. Bumped the top-level README repo-layout sketch range from 01–14 to 01–16. `.pio/` build artifacts stay out (already git-ignored). Numbering is contiguous — 15 is the i2c-scanner added during the I²C-fault diagnosis.

**Files.** `firmware/test-sketches/16-float-divider/platformio.ini` (new), `firmware/test-sketches/16-float-divider/src/main.cpp` (new), `firmware/test-sketches/16-float-divider/README.md` (new), `README.md`.

---

<a id="dl-192"></a>
### DL-192 — Fix the stale `fsm.h` states-list comment

**Date:** 2026-08-20 · **Status:** Active — the DL-190 follow-up; closes the last known stale comment. Comment-only, no reflash.

**What was wrong.** `fsm.h`'s header docstring still described the FSM from the original DL-046 top-water design: it listed states `monitoring, watering, manual, reservoir_empty, daily_limit, leak_fault, stopped, watering_fault, maintenance` — several of which (`monitoring`/`watering`/`manual`/`daily_limit`/`watering_fault`) no longer exist after the P1-8 bottom-watering port (DL-147) — and referred to the pump as `(stubbed)`, which it no longer is. A reader of the header would have been actively misled about the module's states.

**Fix.** Rewrote the docstring to the actual current states, verified line-by-line against the enum and the `state_name()` published strings: `monitor, dosing, settle, grace, reservoir_empty, recovery_hold, sensor_fault, leak_fault, stopped, maintenance`. Updated the accompanying notes — safety (leak, disconnect, stop, the hard pump-on ceiling) evaluated first and overriding; faults latch until ACK; reservoir-empty and recovery_hold as recoverable blocks; maintenance boot-default-ON (DL-128), toggled by MANUAL long-press (DL-089) or remotely (DL-182) — and corrected "(stubbed) pump" to the real pump.

**Deliberately left alone.** The other comment flagged in the DL-190 follow-up, `fsm.cpp:15`, is **not** stale on re-reading: it names the old `ST_WATERING/MANUAL/DAILY_LIMIT/WATERING_FAULT` states as precisely what the P1-8 port *replaced* — correct migration history, not a stale claim. Rewriting it would have erased useful context.

**Verification.** No `monitoring`/`daily_limit`/`ST_WATERING`/`stubbed` references remain in `fsm.h`; braces balanced; the change is comments-only, so the compiled firmware is byte-identical (no reflash). `git diff` touches `fsm.h` only.

**Files.** `firmware/integrated/src/fsm.h`.

---

<a id="dl-193"></a>
### DL-193 — Docs polish sweep (portfolio pass)

**Date:** 2026-08-20 · **Status:** Active — documentation only; no code, tests, or behavior touched.

**Goal.** A desk-cleanup pass to make the repo read as clean, human, and consistent for a portfolio audience — free of LLM-slop tells (em-dash overload, filler words), typos, and terminology inconsistencies.

**Scan result (reassuring).** The portfolio-facing prose was already in good shape: zero filler words (no "comprehensive / robust / seamless / leverage / delve / crucial / testament / …") across `README.md`, `SECURITY.md`, `docs/ai-use.md`, and the sketch READMEs; no typos or doubled words; and the README's em-dashes are overwhelmingly the legitimate `**Term** — definition` bullet convention rather than a stylistic tic.

**Fixes applied.**
- Standardized `grow light` → `grow-light` (chosen house style): `README.md` ×3, `firmware/test-sketches/14-xiao-cam/README.md` ×1.
- Hyphenated the one stray `bottom watering` → `bottom-watering` (README Phase 5 row) to match usage elsewhere.
- Tightened one genuinely convoluted sentence (README "Device-presence awareness"): the nested `— … —` aside became a parenthetical, so the main clause reads in one pass. Meaning and voice preserved.

**Deliberately not done (and why).**
- **The decision log's ~1,436 em-dashes were left untouched.** It is the append-only permanent record, and em-dashes there serve too many grammatical roles to bulk-replace safely — even at high accuracy, a blanket pass would inject dozens of grammatical errors into history for a surface reviewers don't scrutinize for prose style. Agreed path: leave the historical entries, write future entries with lighter, more varied punctuation.
- `WiFi` left as-is (internally consistent; not a real inconsistency).
...
**This piece — the capture trigger.** The camera node was self-timed (a free-running `millis()` interval, no clock, no inbound path) and had no MQTT. To hit precise clock windows coordinated with the light, capture is now **Pi-triggered** (Option a): a single authority on the Pi drives "light off → capture → light on" with no cross-clock drift (the same single-authority lesson as moving photoperiod enforcement to the Pi).

**Firmware changes.**
- New `net_mqtt.{h,cpp}` mirroring the WROVER: connects to the broker (`10.6.19.139:1883`) as client id `camera`, retained presence on `plant/status/camera` + Last-Will `{"online":false}`, subscribes to `plant/cmd/capture`.
- The inbound callback **only latches** a one-shot request + copies the payload as the capture *context* (`dark`/`lit`); the blocking capture+POST runs in the main loop, never inside the PubSubClient callback (which would starve keepalive) — the same pattern as the WROVER's remote start/abort one-shots.
- `poster_post_jpeg` gained a `context` arg, sent as the `X-Capture-Context` header so the receiver can tag frames (Piece 2 will read it).
- `main.cpp` refactored: a `do_capture(context)` helper shared by the MQTT trigger (primary) and the fallback timer.
- **Fallback (D2):** the self-timer is kept but slowed to 2 h and its frames are tagged `fallback`, so if the Pi orchestrator dies the node still captures occasionally but those uncoordinated frames never feed the calibrated metric.
- `platformio.ini`: added `knolleary/PubSubClient@^2.8` (matches the WROVER); `secrets.h.example` gained `MQTT_USER`/`MQTT_PASSWORD`.
...
<a id="dl-197"></a>
### DL-197 — Vision phase, Piece 2a: receiver context-tagging

**Date:** 2026-08-24 · **Status:** Active — hub (image receiver). Deploy: scp + restart.

**Why.** DL-196 made the node send `X-Capture-Context` (`dark`|`lit`|`fallback`). The receiver has to persist it so the metric/analysis step can keep only the grow-light-OFF `dark` frames (the glare-free measurement set) and treat `lit` as the time-lapse record.

**Changes (`hub/09-camera/image_receiver.py`).**
- **Schema:** added `context TEXT` to `camera_readings` through the existing idempotent `ALTER TABLE ... ADD COLUMN` loop (the DL-079 pattern that added `green_area`/`green_ratio`), so the live DB upgrades in place on the next `init_db()` with no manual migration.
- **POST handler:** reads `X-Capture-Context`, defaulting an absent/blank header to `"unknown"` — so a frame from old firmware or a manual `curl` is never silently treated as calibration-grade `dark` data. Passes it to `record()`, and echoes it in both the JSON response and the serial log.
- **`record()`:** now stores the context column.
- **Storage policy (D2):** all in-window captures are stored regardless of context; filtering to `dark` is deferred to the metric/analysis step (Step 3). The photoperiod gate (DL-082) is unchanged — it still drops out-of-window captures, and dark-window captures are inside daytime so they pass.

**Backward compatibility.** Every reader of `camera_readings` selects explicit columns — the DL-194 dashboard query (`ts, path, greenness, green_area, green_ratio`), `camera.py` (`ts, green_ratio, green_area`), and `alerter.py` (`MAX(ts)`) — none use `SELECT *`, so the new trailing column shifts no positional unpack. The DL-195 regression tests (which extract the dashboard query) still pass.

**Docs.** Updated `hub/09-camera/README.md`: the `POST /image` description now documents the `X-Capture-Context` header, the `context` field in the response, and the stored column.

**Verification.** `py_compile` clean; full suite 73 Python + 31 C++ green. Deploy: `scp image_receiver.py` to the Pi, restart `plant-image-receiver`; the `context` column auto-migrates on startup.
...
<a id="dl-198"></a>
### DL-198 — Vision phase, Piece 2b: camera-node firmware doc-banner cleanup

**Date:** 2026-08-24 · **Status:** Active — firmware docs/comments only. **No reflash** (binary byte-identical).

**Why.** DL-196 changed the camera node from a self-timed capturer to a Pi-triggered one, but the file-level banners still described the old model — a reader of the firmware would have been misled. This is the paired doc cleanup for that code change (the split-out "2b" agreed with the "2a" receiver work in DL-197).

**Stale claims corrected.** Across `src/main.cpp`, `platformio.ini`, and `README.md` (two spots): "on a timer" / "cadence hourly" / "node carries no clock" (capture is now Pi-triggered — the Pi owns the clock), and "the MQTT capture event / presence is a later slice" (MQTT arrived in DL-196). Rewrote all four to the current model: Pi-published `plant/cmd/capture` trigger, per-frame context tagging, the self-timer kept only as a resilience fallback, presence over MQTT. Preserved the still-accurate context — DL references, WiFi-loss-non-fatal, Pi-side photoperiod gating (DL-082) — and the genuinely-still-future DL-059 watchdog slice (noting it needs a cadence-aware timeout because triggered captures are irregular).

**Verification.** `grep` confirms no old-model language remains (only the legitimate "watchdog is a later slice" and the "Not yet" section survive). The `.cpp` and `.ini` diffs contain **zero non-comment lines**, so the compiled firmware is byte-identical — no reflash. Test suite (73 Python + 31 C++) unaffected; no hub/Python touched.

**Files.** `firmware/camera-node/src/main.cpp`, `firmware/camera-node/platformio.ini`, `firmware/camera-node/README.md`.

---

<a id="dl-199"></a>
### DL-199 — Vision phase, Piece 3: photoperiod dark-window integration

**Date:** 2026-08-24 · **Status:** Active — hub (grow-light enforcer + a receiver comment). **Validated on hardware 2026-08-26.**

**Why.** The measurement captures must be taken with the grow-light OFF (the close light throws specular highlights that read near-white, ExG ≈ 0, corrupting the green metric). The Pi photoperiod enforcer runs every ~2 min and *wants the light ON* all day, so it would fight any attempt to go dark. This piece makes the 5 dark windows part of what the enforcer considers *correct*, so it holds the light off during them instead of "correcting" it back on.

**Design.**
- **Shared config, single source of truth (D1).** `DARK_WINDOW_CENTERS` (default `08:00,10:00,12:00,14:00,16:00`) + `DARK_WINDOW_LEAD_MIN` (2) + `DARK_WINDOW_TAIL_MIN` (3), read from the environment like the rest of the enforcer config. The Piece-4 orchestrator will read the same centers and fire its capture at the center; defining the windows once means the enforcer and orchestrator cannot drift.
- **`desired_on(hour)` unchanged.** The base photoperiod stays exactly as it was (all its existing tests pass untouched). The dark window is layered as a separate concern: new `in_dark_window(now)` (minute-precise; a window is `[center-LEAD, center+TAIL]` inclusive; centers parsed lazily so `log()` exists for malformed-entry warnings) and `grow_light_should_be_on(now) = desired_on(now.hour) and not in_dark_window(now)`.
- **`main()`** now decides via `grow_light_should_be_on` and logs a dark-window hold distinctly ("dark capture window ... holding grow-light OFF") from a genuine correction, so the journal doesn't misread an intentional dark hold as the plug misbehaving.

**Receiver docstring corrected.** `image_receiver.within_photoperiod` said it "mirrors the enforcer's desired_on() exactly" — now false. It intentionally stays hour-based (07:00-19:00) and does NOT track dark windows, because a dark-window frame is taken during the day and must be *stored* (it is the measurement set). The two functions now answer different questions; the docstring says so. (No behavior change in the receiver — dark-window captures are in daytime and already pass its gate.)

**Timing note for Piece 4.** With a 2-min timer and a 5-min window the enforcer reliably ticks inside each window, but exact tick phase can vary, so the orchestrator (Piece 4) should confirm the plug is actually OFF (read-only query — not a control action, so it does not violate the enforcer being sole plug *setter*) before capturing, or skip that window, rather than assuming the light is off at the center instant.

**Verification.** `py_compile` clean. 8 new tests (window centers, inclusive boundaries, between-window gaps, day+window combination, night, malformed-center tolerance), mutation-verified (nulling `in_dark_window` fails them). Full suite 81 Python (73 + 8) + 31 C++ green. Deploy: scp `photoperiod.py` and `image_receiver.py`, restart `plant-photoperiod` (picks up on the next timer tick) and `plant-image-receiver`.

**Files.** `hub/08-grow-light/photoperiod.py`, `hub/09-camera/image_receiver.py`, `tests/test_photoperiod.py`.

**Validated (2026-08-26).** Deployed while away; the enforcer's journal for the day shows all five dark windows firing as clean hold-OFF → restore-ON pairs: OFF at 07:58/09:58/11:58/13:58/15:58 (each logged "dark capture window ... holding grow-light OFF (was ON) -> set OFF") and back ON at 08:04/10:04/12:04/14:04/16:04, plus the normal 19:00 end-of-day shutoff. "(was ON)" on each hold confirms the enforcer actively turned a genuinely-on light off (not a no-op), and the distinct log lines read clearly separate from real corrections. Live behavior matches the design: ~2.5-3 min of glare-free dark per window, centered on the :00 capture instant.

---

<a id="dl-200"></a>
### DL-200 — Vision phase, Piece 4: the capture orchestrator

**Date:** 2026-08-26 · **Status:** Active — new hub service. Deploy: install + enable timer.

**What.** `hub/13-capture-scheduler/` completes the vision-phase capture pipeline. A `oneshot` service, run every minute by `plant-capture-scheduler.timer` (`OnCalendar=*:0/1`), publishes `plant/cmd/capture` with the context the node echoes and the receiver tags:
- **`dark`** at each dark-window center (08:00/10:00/12:00/14:00/16:00) — the glare-free measurement frames that feed the calibrated metric;
- **`lit`** every 30 min (`:00`/`:30`) through the lit day — the time-lapse/visual record, not fed to the metric;
- **nothing** outside the lit day — the node's own 2 h fallback covers liveness.

**Single authority, pure trigger (D1).** The orchestrator never sets the plug — the photoperiod enforcer (DL-199) is the sole setter. Before a `dark` capture it *reads* the plug (a read-only `Switch.GetStatus` RPC) and triggers only if the light is confirmed OFF; if the enforcer hasn't caught up (tick-phase variance against the 5-min window) it **skips the window and logs** rather than capturing a lit frame mislabelled `dark`. Missing an occasional dark capture is harmless (five windows/day, the metric tolerates gaps); making the orchestrator a second plug-controller would reintroduce the dual-authority fragility the project avoids. This is the robustness approach flagged in DL-199.

**No drift.** The dark centers are read from the same `DARK_WINDOW_CENTERS` env as the enforcer — one source of truth for both services. A dark center takes precedence over the coincident `:00` lit slot.

**Design.** `decide(now, centers) -> "dark"|"lit"|None` is a pure function (no I/O), which is what the tests exercise; `light_is_off()` (read-only plug query) and `publish()` are the I/O around it. `publish()` mirrors the dashboard's `mqtt_publish.single(topic, ctx, hostname=..., auth=..., retain=False)` — `retain=False` is essential, since a retained capture command would be replayed to the node on every broker reconnect.

**Verification.** `py_compile` clean. 8 tests: dark centers, dark-wins-over-lit precedence, lit `:00`/`:30` cadence, a lit capture on the half-hour inside a dark-window *hour*, non-slot minutes / inside-window-but-not-center = nothing, night = nothing, day boundaries (ON inclusive / OFF exclusive), malformed-center tolerance. Mutation-verified (swapping the dark/lit precedence fails the right tests). Full suite 89 Python (81 + 8) + 31 C++ green. (A stale `.pyc` from the in-place mutation test briefly masked the restore — cleared `__pycache__`; the source was correct throughout. Lesson: clear bytecode caches after mutating source in place.)
...
<a id="dl-201"></a>
### DL-201 — Fix DL-200 orchestrator crash: wrong Python interpreter

**Date:** 2026-08-26 · **Status:** Active — one-line service fix. Deploy: re-copy unit + restart.

**Symptom.** `plant-capture-scheduler.service` failed on every timer tick with `ModuleNotFoundError: No module named 'paho'` (traceback at `import paho.mqtt.publish`).

**Cause.** The service's `ExecStart` used `/usr/bin/python3`, copied from the `plant-photoperiod` / `plant-retention` service files. But those two are **stdlib-only** (`urllib`, `json`, `datetime`) so system Python is fine for them; the orchestrator imports **`paho`**, which is installed in the project virtualenv (`/home/basilpi/plant-hub/venv/`), the same interpreter the dashboard and listener use. So system Python couldn't import it and the service crashed before doing anything.

**Fix.** Point `ExecStart` at `/home/basilpi/plant-hub/venv/bin/python`. Documented the rule in `hub/13-capture-scheduler/README.md`: stdlib-only services use system `python3`; anything importing `paho` (or other venv deps) must use the venv interpreter. The hub's service interpreter is not uniform, and copying a stdlib-only service as a template is the trap.

**Caught early.** The post-deploy `journalctl` check surfaced it the same night. It was harmless overnight (outside the lit day the orchestrator publishes nothing), but unfixed it would have meant zero dark/lit captures the next day — the whole point of the pipeline. No repo bug in the Python itself; purely the unit-file interpreter.
...
<a id="dl-203"></a>
### DL-203 — Raise the watering band 20-40% -> 60-80%

**Date:** 2026-09-02 · **Status:** Active — firmware (thresholds). **Awaiting reflash of the WROVER.**

**Why.** The plant was chronically under-watered. Two weeks of `sensor_readings` told a clear story: a watering on Aug 20 took soil to ~71%, then a remarkably linear ~2.5%/day dry-down over **13 straight days** with no re-watering — 68.7 -> 63 -> 58 -> ... -> 34 -> **28.7% today** — because it never fell to the old 20% `TRIGGER_PCT` floor. Physical inspection confirmed water stress: browning leaf edges, dried leaves, and curled-but-green leaves (a water-need sign). The old band was simply too dry for this plant at its current size.

**What the data also settled.** The Aug-20 ~71% was drawn down healthily over two weeks with no waterlogging or rot, so the plant clearly tolerates and prefers the higher range — which removed the earlier worry that an 80% target risked waterlogging. The plant visibly thrived in the 50s-60s and showed stress only approaching ~30%.

**Change.** `TRIGGER_PCT` 20 -> 60, `TARGET_PCT` 40 -> 80 (chosen with Temmy: a 60% trigger keeps it up in the range it thrives in, with a 20-point band above a 100mL first dose). `config.h` only; dosing constants (DOSE1 100, SUPPLEMENT 50, SESSION_CAP 600, MAX_DOSE 150) unchanged. Updated the band comment (was "20-40% band") and the EQUILIBRATED-OUTCOME note to reference `TARGET_PCT` generically.

**Immediate effect.** At 28% the plant is far below the new 60% trigger, so it starts a watering cycle on the first FSM tick after flashing — relieving the overdue drink.

**Watch the first cycle(s) — genuinely unknown behavior.** The only equilibration data we have (100mL -> ~70%, DL-154) was measured against the *old* 40% target. With an 80% target the supplement loop will keep dosing 50mL until the plateau reaches 80%, and delayed wicking could push the final equilibrium past 80% (into the mid-80s). That is not dangerous (the plant handled ~71% fine) but it is unmeasured, so the first cycle or two on the new target should be observed to see where it actually settles before deciding whether DOSE1/SUPPLEMENT need tuning. Don't pre-tune blind — change thresholds, observe, adjust if warranted.

**Verification.** `git diff` confirms only the two thresholds and their comments changed (no logic); `water_logic.h` takes thresholds as parameters (no hardcoded values), so the pure logic is unaffected; braces/parens balanced; the 31 C++ `water_logic` host checks pass. Cannot compile ESP32 firmware here — **reflash the WROVER** (`pio run -t upload`), then confirm the FSM triggers a cycle (soil 28% < 60%) and watch where it equilibrates.
...
<a id="dl-204"></a>
### DL-204 — Fix premature watering fault on drought recovery (ABSORB_RISE_PCT 7->2)

**Date:** 2026-09-03 · **Status:** Active — firmware (one threshold + tests). **Awaiting reflash of the WROVER.**

**Symptom.** After DL-203 raised the band to 60/80, the first watering of the drought-stressed plant faulted after a single 100mL dose. Telemetry at the fault: `dose_count 1`, `session_ml 100`, `moist_ema 30.7`, state `stopped` — i.e. it dosed once, moisture rose ~28->31 (~3%), and it gave up instead of supplementing toward 80%.

**Root cause.** `evaluate_decision` classifies a dose as "absorbed" only if the rise >= `ABSORB_RISE_PCT` (was 7%); a smaller rise is treated as a stall -> GRACE -> STOP_FAILED (the dead-pump / empty-reservoir signature). But a 100mL dose into **bone-dry media** moved the probe only ~3% — real absorption that fell under the 7% bar, so a genuinely-thirsty-but-working plant was misread as a hardware failure. The 7% figure was over-fit to the normal moisture range; the probe swings less per mL when the media is very dry (and, symmetrically, near saturation).

**Corroborating data.** A later cycle in the normal range dosed 100mL, rose >=7%, correctly took the SUPPLEMENT path, and reached ~80% in **2 doses / 150mL** with no fault. So the supplement logic itself is sound — only the low-end threshold was wrong. The bug bites specifically in drought recovery, which recurs (outages, missed waterings), so it is worth fixing rather than treating as a one-off.

**Fix.** `ABSORB_RISE_PCT` 7 -> 2. Any real positive rise (>=2%) now counts as absorption and supplements toward target; only a near-zero rise (~0%, the true failure signature) still stalls -> GRACE -> STOP_FAILED. The `DONE`-on-target check runs first, so the near-saturation case (small rise because the pot is nearly full) still stops correctly on reaching 80%, not as a stall. **Number chosen for asymmetric cost:** a false stall strands a thirsty plant and needs manual intervention (what happened here); a false "absorbed" adds a single 50mL supplement bounded by `SESSION_CAP_ML`. Erring toward watering is the safe direction.

**Tests.** Kept the existing logic tests (which parametrize the threshold). Added a DL-204 block exercising the *production* 2.0 threshold with the real numbers: 3% dry-recovery rise -> SUPPLEMENT (the exact bug), 2.5% marginal -> SUPPLEMENT, 8% normal -> SUPPLEMENT, ~0.5% failure -> GRACE then STOP_FAILED, near-target small rise -> DONE. Mutation-verified: reverting the test threshold to 7.0 fails the 3% and 2.5% cases, so a future re-raise cannot pass silently. 37 C++ checks + 95 Python green; `config.h` braces balanced.

**Files.** `firmware/integrated/src/config.h`, `tests/firmware/test_water_logic.cpp`.
...
**Isolating it.** An on-demand test removed every variable: manually switched the plug off (`{"was_on":true}` -> off, and physically watched the grow-light go dark), triggered a capture, switched it back on. The plant was visibly dark, but the captured frame was still lit. Physical-dark + image-lit means the image did not reflect the moment of capture -- the node returned a **stale frame** buffered before the light went off. This also retroactively explains every earlier "dark" frame looking lit; scheduling, clock, enforcer and plug were not the cause (the timezone issue fixed earlier today was a separate, real bug).

**Root cause.** `camera.cpp` initialised the sensor with `grab_mode = CAMERA_GRAB_WHEN_EMPTY` and `fb_count = 2` (PSRAM). In that mode `esp_camera_fb_get()` returns the oldest *queued* frame rather than capturing the current scene, and `camera_capture()` did a single `fb_get()` with no flush. With the camera continuously filling both buffers while the light was on, a capture taken just after the light went off handed back a queued *lit* frame.

**Fix.** `grab_mode = CAMERA_GRAB_LATEST` (bias `fb_get()` toward the newest frame and drop stale ones), and `camera_capture()` now **drains the queued buffers before the returned grab**: it grabs and immediately releases `fb_count` frames, with a 120 ms delay between them so the sensor's auto-exposure/gain re-adapts to the darker scene, then grabs the fresh frame it returns. The explicit flush makes freshness deterministic rather than relying on grab_mode alone. Framebuffer ownership is unchanged -- each flushed frame is returned, and the final frame is released by the caller in `main.cpp`.

**Cost.** ~240 ms + exposure settle per capture (two throwaway frames). Negligible for the 5-dark + 30-min-lit cadence, and it runs in the loop's `do_capture()` path, not the MQTT callback, so it never starves keepalive.
...
<a id="dl-206"></a>
### DL-206 — Purge corrupted/test camera rows from plant.db

**Date:** 2026-09-03 · **Status:** Active — live-database hygiene op (no code/repo change).

**Why.** DL-205 fixed the stale frame-buffer bug, but the `camera_readings` table still held the data that bug produced. Since the dark-frame set is destined for metric calibration (and later possibly ML), leaving corrupted frames in it risks silently training/calibrating on garbage -- a `WHERE` filter is one forgotten clause away from contamination. Cleaner to delete the known-bad rows outright.

**Deleted (44 rows + their JPEG files).**
- **33 `dark` rows** (2026-08-27 → 09-03): every pre-DL-205 "dark" capture was actually a stale buffered *lit* frame (see DL-205). None are real dark measurements, so all were removed.
- **`darktest` + `darktest2`** (2 rows): the manual on-demand test captures used to diagnose DL-205.
- **9 `fallback` rows**: node self-timer captures under uncontrolled lighting -- never valid for the calibrated metric.

**Kept (deliberately).**
- **126 `lit` rows**: the 30-min time-lapse record (grow-light on) -- legitimate visual/portfolio data.
- **948 empty-context rows** (2026-06-22 → 08-25): the pre-tagging growth baseline from the original hourly-capture design. Two months of continuous history -- valuable as a growth record, and the empty context cleanly distinguishes them from any dark-frame query, so they can never contaminate a `context='dark'` calibration filter. (Empty context is itself the honest label: these predate the dark/lit distinction, so their lighting condition is simply unknown and was never backfilled.)

**How (safe, staged).** SELECT-confirmed the exact 44 targets before deleting; wrote their paths to a file and removed the image files first (44 deleted, 0 missing); then `DELETE ... WHERE context IN ('dark','darktest','darktest2','fallback')` with `SELECT changes()` = 44; verified after-state = only `lit`(126) + empty(948).

**Net effect.** From the DL-205 fix onward, `context='dark'` contains only genuinely-dark frames. Anyone doing calibration/ML on the dark set can trust every row without a date filter; the pre-Sep-4 absence of dark frames is explained here.

**Files.** None (live `plant.db` on the Pi; no repository change).

<a id="dl-207"></a>
### DL-207 — README currency pass (vision phase + watering changes)

**Date:** 2026-09-03 · **Status:** Active — documentation only (top-level README).

**Why.** The README still described the pre-vision-phase system and the old watering band. With the dark-window capture pipeline built and validated (DL-196–205) and the band raised (DL-203/204), the showcase doc had drifted from reality.

**Changes (scope agreed first).**
- **"Next" roadmap row** — reworded from "Vision analysis ..." to reflect that the grow-light-off measurement-capture pipeline is *built + hardware-validated*, and what actually remains is greenness-metric **calibration** on those frames, then growth/health analysis and adaptive lighting.
- **Autonomous-watering bullet** — states the current 60→80% band and why it was raised (two weeks of dry-down data showed chronic under-watering, DL-203); DL range refreshed 104–188 → 104–204.
- **Grow-light bullet** — adds that the enforcer also holds the light **off** during five short daily measurement windows so the top-down camera captures free of specular glare (DL-199).
- **Repo layout** — hub range `01…11-shelly-monitor` → `01…13-capture-scheduler`, and the description now lists the capture orchestrator.
- **Push-alerting bullet** — adds the 9 pm end-of-day summary reporting the day's net soil-moisture change (DL-202) alongside the morning heartbeat.

**Deliberately not done.** Left the Phase 4 table row unchanged — it accurately describes the original camera bring-up, which *was* complete; the dark-window rework is reflected in the "Next" row and the grow-light/layout updates rather than by rewriting history. Framed the capture architecture as done and the calibration/analysis as upcoming — honest, since the clean pipeline is one day old and multi-day collection has not yet run. No terminology regressions (grow-light hyphenation held).
