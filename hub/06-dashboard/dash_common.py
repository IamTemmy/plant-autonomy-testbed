...
def daily_ml_delivered():
    """mL delivered in the last 24 h, summed from session_ml positive deltas (each
    dose bumps session_ml; a reset drops it). Both firmwares publish session_ml
    (harness P1-6, integrated DL-148). Returns 0.0 with no session data.
    F9 (DL-175): dropped the old daily_pump_ms fallback (retired concept, DL-150/158)."""
    df = query_df(
        """WITH s AS (
    ...
    return float(df.iloc[0]["ml"]) if not df.empty and df.iloc[0]["ml"] is not None else 0.0
