...
def route_message(
    ...
    try:
        ...
        if len(parts) == 3 and parts[1] == "state":
            ...
            if maint is not None:
                conn.execute(
                    """INSERT INTO system_status
                       (ts, message_id, run_id, device, status, metric, value)
                       VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (ts, message_id, run_id, device, "on" if maint else "off",
                     "maintenance", 1.0 if maint else 0.0),
                )
            if pump is not None:
                conn.execute(
                    """INSERT INTO system_status
            ...
            # Bottom-watering loop alerts (DL-109): fire on the WROVER's session
            # reason. Both firmwares publish a reason (integrated since the P1-8 port;
            # F1/DL-163 restored the stop reasons), so this drives real alerts.
            if device == "wrover":
